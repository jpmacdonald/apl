...
| **`apl-core`** | **Registry** | Aggregates artifacts from `apl-ports` and upstream GitHub Releases into a unified Index. Publishes the Index and Edge API. |
...
*   **Inputs:**
    ...
    2.  **Ports Feed:** Scans the `r2.apl.pub/ports` bucket for artifacts built by `apl-ports` (5% of packages).
*   **Indexing strategy:**
...
*   `/index`: The global binary index generated by `apl-core`.
...
2.  **Pointer (`apl-core/registry/ruby.toml`)**

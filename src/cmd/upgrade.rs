...
pub async fn upgrade(packages: &[String], dry_run: bool) -> Result<()> {
    ...
    for pkg_name in &to_check {
        ...
        let Some(installed_pkg) = installed_pkg else {
            if !packages.is_empty() {
                output.warning(&format!("{pkg_name} is not installed"));
            }
            continue;
        };
        ...
        if installed_pkg.version != index_entry.latest().version {
            upgrades.push((
                ...
                index_entry.latest().version.clone(),
            ));
        }
    }
    ...
    output.section("Upgrades available");
    for (name, old, new) in &upgrades {
        println!("  {:<14} {} → {}", name, old, new);
    }
    ...
    // Perform upgrades
    let upgrade_names: Vec<String> = upgrades
        .into_iter()

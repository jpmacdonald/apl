...
impl apl_ports::Strategy for HashiCorpStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
        ...
        for (version, data) in resp.versions {
            if version.contains('-') { continue; } // Skip unstable
            
            // Fetch SHA256SUMS for this version
            // https://releases.hashicorp.com/terraform/1.5.0/terraform_1.5.0_SHA256SUMS
            let sha_url = format!("https://releases.hashicorp.com/{}/{}/{}_{}_SHA256SUMS", 
                self.product, version, self.product, version);
            
            let shas = fetch_shasums(&self.client, &sha_url).await.unwrap_or_default();
...
#[async_trait]
impl apl_ports::Strategy for GolangStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
...
#[async_trait]
impl apl_ports::Strategy for NodeStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
        ...
        let mut artifacts = Vec::new();
        for release in releases.iter().take(50) { // Limit to 50 for MVP
            ...
            let shas = fetch_shasums(&Client::new(), &sha_url).await.unwrap_or_default();
...
#[async_trait]
impl apl_ports::Strategy for GitHubStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
        // Placeholder
        Ok(vec![])
    }
...
#[async_trait]
impl apl_ports::Strategy for AwsStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
        ...
        for line in text.lines() {
            if let Some(cap) = re.captures(line) {
                 if line.trim() == &cap[1] { // likely a header line
        ...
        for version in versions.iter().take(5) {
             ...
             let sha256 = download_and_hash(&Client::new(), &url).await
                .unwrap_or_else(|e| {
...
#[async_trait]
impl apl_ports::Strategy for PythonStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
         ...
         let mut versions: Vec<String> = re.captures_iter(&text)
             .map(|c| c[1].to_string())
             .collect();
         ...
         for version in versions.iter().take(5) {
             ...
             // Fetch SHA
             let sha256 = download_and_hash(&Client::new(), &url).await.unwrap_or_default();
...
#[async_trait]
impl apl_ports::Strategy for RubyStrategy {
    async fn fetch_artifacts(&self) -> Result<Vec<Artifact>> {
